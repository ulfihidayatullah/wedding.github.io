...
<div class="container">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <?php
            $result = mysqli_query($conn, "SELECT DISTINCT invoice, kode_customer, status, order_date, kode_produk, terima,tolak, cek FROM produksi group by invoice");
            $no = 1;
            $array = 0;
            ...
            ?>
                ...
                <tr>
                    ...
                    <?php if ($row['terima'] == 1) { ?>
                        <td style="color: green;font-weight: bold;">Pesanan Diterima
                        <?php
                    } else if ($row['tolak'] == 1) {
                        ?>
                    ...
                    while ($row1 = mysqli_fetch_assoc($t_bom)) {
                        ...
                        <td><?= $tanggal = $row['order_date']; ?></td>

                        <td>
                            <?php if ($row['tolak'] == 0 && $row['cek'] == 1 && $row['terima'] == 0) { ?>
                                <a href="proses/terima.php?inv=<?= $row['invoice']; ?>" class="btn btn-warning"><i class="glyphicon glyphicon-warning-sign"></i> Terima</a>
